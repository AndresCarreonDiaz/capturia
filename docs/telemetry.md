...
- `installId`: a random UUID minted once on first launch and stored in
  `~/Library/Application Support/Capturia/settings.json`. It identifies an
  installation so unique installs can be counted; it is connected to nothing
  else (no account, no email, no hardware id).
...
- `appVersion`: the Capturia version, for adoption-per-release counts.
- `macosVersion`: the macOS version, for support decisions.
...
| `beacon:versions` | hash of appVersion to launch count (capped at 200 fields) | none |
