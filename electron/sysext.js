...
//   onInstalled()         fires once when an activation request completes,
//                         the cue to nudge the camera feed to connect.
...
function createSysext({ onStateChange, onInstalled, offerMove, log = console } = {}) {
  ...
  let list = { present: false, enabled: false, awaitingApproval: false };
  ...
  // machine-state half of the snapshot. Detection failure means "unknown",
  // which parses to not-installed rather than throwing.
  function refresh() {
    return new Promise((resolve) => {
      execFile("systemextensionsctl", ["list"], { timeout: 5000 }, (err, stdout) => {
        if (!disposed) {
          list = parseSystemExtensionsList(err ? "" : String(stdout || ""));
          notify();
        }
        resolve(getState());
      });
  ...
  // exactly what AMFI checks when the request is submitted.
  ...
  // Fire the activation request. Options exist for the packaged smoke only:
  // force bypasses the already-enabled short-circuit (to exercise the real
  // request path on a machine where the extension is live), and onEvent taps
  // the raw delegate events. Product callers (IPC, tray) pass nothing.
  function install({ force = false, onEvent } = {}) {
    ...
    if (!supported || request.phase !== "idle" || (list.enabled && !force)) {
